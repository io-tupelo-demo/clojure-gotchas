...
; rest-destructuring first places the map into a list like [m], then unwraps
; the map from the list before pulling out the values for x, y, & m
(defn bar
  [a b & {:keys [x y] :as m}]
  (is= [a b] [1 2])
  (is= [x y] [3 4])
...
(defn bazz
  ...
  (let [{:keys [x y] :as m} {:x 3 :y 4}]
    (is= [x y] [3 4])
    (is= m {:x 3 :y 4}))

  ; note that destructuring SILENTLY UNWRAPS the map from inside the list
  (let [{:keys [x y] :as m} (list {:x 3 :y 4})]
    (is= [x y] [3 4])
    (is= m {:x 3 :y 4}))

  ; a vector IS NOT silently unwrapped
  (let [{:keys [x y] :as m} (vector {:x 3 :y 4})]
    (is= [x y] [nil nil])
    (is= m [{:x 3 :y 4}])) ; the map is still wrapped in a vector

  ; a seq is SILENTLY UNWRAPPED just like a list
  (let [{:keys [x y] :as m} (seq (vector {:x 3 :y 4}))]
    (is= [x y] [3 4])
    (is= m {:x 3 :y 4})))
